...
const mailPath = "./commands/ticket/";
...
function readFiles() {
  ...
  const proxyJsFiles = fs
    .readdirSync(proxyPath)
    .filter((f) => f.endsWith(".js"));
  const privateJsFiles = fs
    .readdirSync(staffPath)
    .filter((f) => f.endsWith(".js"));
  const mmJsFiles = fs.readdirSync(mailPath).filter((f) => f.endsWith(".js"));

  const helpCommands = helpJsFiles
    .map((cf) => {
      try {
  ...
  const miscCommands = miscJsFiles
    .map((cf) => {
      try {
        // remove the require cache for the command module
  ...
  const proxyCommands = proxyJsFiles
    .map((cf) => {
      try {
        //remove the require cache for the command module
  ...
  const privateCommands = privateJsFiles
    .map((cf) => {
      try {
        // remove the require cache for the command module
...
function deploy(log = false) {
  ...
  rest
    .put(Routes.applicationGuildCommands(self, home), { body: privateCommands })
    .then((res) => {
  ...
  rest
    .put(Routes.applicationCommands(self), {
      body: [
        ...helpCommands,
        ...miscCommands,
        ...modCommands,
