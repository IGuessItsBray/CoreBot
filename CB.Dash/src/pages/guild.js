...
export default class Guild extends Component {
	...
	render() {
		return (
			<>
				<PageLoading loading={this.state.loading} text={`Loading ${this.state.guild.name}`} />
				<button class='btn btn-sm text-muted' onClick={() => this.props.setPage('guilds')}>←Back to Guilds</button>
				<div class="card">
					<div class="card-header row" style={{ '--bs-gutter-x': '0' }}>
						<div class='col-auto'>
							<img
								...
								alt={`${this.state.guild.name} icon`} width="10" />
						</div>
						<div class='col'>
							<h5 class="card-title">{this.state.guild.name}</h5>
							<h6 class="card-subtitle">{this.state.guildPage}<br /><code class="mb-2 text-muted">{this.state.guild.id}</code></h6>
						</div>
					</div>
					<div class="btn-group btn-group-sm" style={{'borderTop':'none', 'borderBottom': '1px solid rgba(0,0,0,.125)', 'borderRadius': 0}}>
						<button type="button" class={`btn btn-light ${this.state.guildPage === 'overview' ? '' : ''}`}  onClick={() => this.setGuildPage('guilds')}>Overview</button>
						<button type="button" class={`btn btn-light ${this.state.guildPage === 'scheduler' ? '' : ''}`} onClick={() => this.setGuildPage('scheduler')}>Scheduler</button>
						<button type="button" class={`btn btn-light ${this.state.guildPage === '' ? '' : ''}`}     onClick={() => this.setGuildPage('stats')}>Stats</button>
					</div>
					<div class="card-body">
						{this.renderSwitch()}
					</div>
